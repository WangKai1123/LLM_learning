...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        out = self.projection_layer(self.projection_layer(head_outputs))
        return out

#Defin transformer block\
...
class Model(nn.Module):
    def __int__(self, max_token_value=100256):
        super().__init__()
    ...
    def generate(self,idx,max_new_tokens=100):
        for _ in range(max_new_tokens):
            ...
            #Get the last time step from logits where the dimenstions of the logits are (B,T,C)
            logits_last_timestep = logits[:,-1,:]
            probs = F.softmax(input=logits_last_timestep,dim=-1)
            # Sample from the proabilities distribution
